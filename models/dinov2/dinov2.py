...
def build_model(args: argparse.Namespace, device: torch.device) -> DINOv2Classifier:
    ...
    configured_size = int(model.backbone.config.image_size)
    if args.image_size != configured_size:
        raise ValueError(
            f"{args.model_name} is configured for {configured_size}x{configured_size} "
            f"inputs, but --image-size is {args.image_size}. Use "
            f"--image-size {configured_size} with this implementation."
        )
